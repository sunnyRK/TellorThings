...
class TradeContainer extends Component {
  ...
  async calculateSLippageRateFromTellorOracle() {
    ...
    let price1;
    if(this.state.token0 == "WETH") {
      price1 = await oracleInstance.methods.getCurrentValueFromTellorOracle("ETH").call(); 
    } else {
      price1 = await oracleInstance.methods.getCurrentValueFromTellorOracle(this.state.token0).call(); 
    }
              ...
              if (this.state.shouldSwap) {
                ...
                // if (parseInt(balance) >= this.state.swapValinWei) {
                  ...
                  if (parseInt(allowance) >= parseInt(this.state.amountSwapDesired)) {
                    ...
                    const transactionHash = await routeContractInstance.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
                      // web3.utils.toWei(this.state.amountSwapDesired.toString(), 'ether'),
                      this.state.minValue,
                      ...
                      from: accounts[0],
                      value: web3.utils.toWei(this.state.amountSwapDesired.toString(), 'ether')
                    });
