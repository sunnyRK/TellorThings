...
class TradeContainer extends Component {
  ...
  async calculateSLippageRateFromTellorOracle() {
    ...
    if(this.state.token1 == "WETH") {
      price2 = await oracleInstance.methods.getCurrentValueFromTellorOracle("ETH").call(); 
    } else {
      price2 = await oracleInstance.methods.getCurrentValueFromTellorOracle(this.state.token1).call(); 
    }
    ...
    console.log("Priceee :", token1Price)
    ...
    // alert(token1Quantity)
    this.setState({
      slippage: slippage2,
      tellorRate: token1Quantity / 1e18,
      tellorRateWithToken: token1Quantity/1e18 + " ETH"
    });
  }
  ...
  async getAmountOutValue() {
    ...
    const reserves = await pairInstance.methods.getReserves().call();
      ...
      this.setState({
        amountOut,
        amountOutWithToken: amountOut + " ETH"
      });
        ...
        if (this.state.token0 != '') {
          ...
          if (this.state.swapValinWei > 0) {
            ...
            if (parseFloat(this.state.amountOut) >= parseFloat(this.state.tellorRate)) {
              ...
              if (parseInt(this.state.slippage) <= 30) {
                this.setState({ shouldSwap: true });
              } else {
              ...
              if (this.state.shouldSwap) {
                ...
                if (parseInt(balance) >= this.state.swapValinWei) {
                  ...
                  if (parseInt(allowance) >= parseInt(this.state.amountSwapDesired)) {
                    ...
                    const transactionHash = await routeContractInstance.methods.swapTokensForExactETH(
                      web3.utils.toWei(this.state.amountSwapDesired.toString(), 'ether'),
  ...
  render() {
    ...
    return (
      <Trade
        ...
        // tagOptionsForETh={tagOptionsForETh}
        tradePairTokens={tradePairTokens}
        tagOptionsForDAI={tagOptionsForDAI}
        ...
        // swapExactETHForTokens={this.swapExactETHForTokens}
        swapTokensForExactETH={this.swapTokensForExactETH}
